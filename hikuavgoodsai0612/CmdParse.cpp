...
int ParserUav(cJSON *pUavArray, UAV *astUav, int *pNum)
{
    ...
    *pNum = cJSON_GetArraySize(pUavArray);
    for(int i = 0; i < *pNum; i++)
    {
        ...
        nRet = JSONGetValue(pUAV, "remain_electricity", false, &(astUav[i].nRemainElectricity));
        if(nRet != 0)
        {
            printf("JSONGetValue error\n");
            return nRet;
...
int CreateFlayPlane(FLAY_PLANE *pstPlane, char *szToken, char *pBuffer, int *pLen)
{
    ...
    {
        if(pstPlane->astUav[i].nStatus == 1)continue;
        ...
        cJSON_AddItemToObject(pUav, "remain_electricity", cJSON_CreateNumber(pstPlane->astUav[i].nRemainElectricity));
